...
function ensureLockDirectoryExists() {
  ...
  if (!_fs.default.existsSync(locksPath)) {
    _fs.default.mkdirSync(locksPath, {
      recursive: true
    });
  }
}
...
function setLock(identifier) {
  ...
  _fs.default.writeFileSync(_path.default.join((0, _index.getPaths)().generated.base, 'locks', identifier), '');
}
...
function unsetLock(identifier) {
  try {
    _fs.default.rmSync(_path.default.join((0, _index.getPaths)().generated.base, 'locks', identifier));
  } catch (error) {
...
function isLockSet(identifier) {
  ...
  const exists = _fs.default.existsSync(lockfilePath);
  if (!exists) {
    return false;
  }
  ...
  const createdAt = _fs.default.statSync(lockfilePath).birthtimeMs;
...
function clearLocks(identifiers = []) {
  ...
  if (identifiers.length > 0) {
    ...
    const locks = _fs.default.readdirSync(_path.default.join((0, _index.getPaths)().generated.base, 'locks'));
