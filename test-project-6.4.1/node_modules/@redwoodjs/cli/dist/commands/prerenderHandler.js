...
function getRouteHooksFilePath(routeFilePath) {
  const routeHooksFilePathTs = routeFilePath.replace(/\.[jt]sx?$/, '.routeHooks.ts');
  if (_fs.default.existsSync(routeHooksFilePathTs)) {
    return routeHooksFilePathTs;
  }
  const routeHooksFilePathJs = routeFilePath.replace(/\.[jt]sx?$/, '.routeHooks.js');
...
const getTasks = async (dryrun, routerPathFilter = null) => {
  ...
  if (prerenderRoutes.length === 0) {
    console.log('\nSkipping prerender...');
    console.log(_colors.default.warning('You have not marked any routes with a path as `prerender` in `Routes.{jsx,tsx}` \n'));
