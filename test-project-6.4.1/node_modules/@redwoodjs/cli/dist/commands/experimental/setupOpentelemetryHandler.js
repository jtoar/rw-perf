...
const handler = async ({
  ...
  const opentelemetryTasks = [{
    ...
    task: () => {
      const setupTemplateContent = _fs.default.readFileSync(_path.default.resolve(__dirname, 'templates', 'opentelemetry.ts.template'), 'utf-8');
    ...
    enabled: () => {
      return _fs.default.existsSync((0, _projectConfig.resolveFile)(_path.default.join((0, _lib.getPaths)().api.functions, 'graphql')));
    },
    task: (_ctx, task) => {
    ...
    enabled: () => {
      return _fs.default.existsSync((0, _projectConfig.resolveFile)(_path.default.join((0, _lib.getPaths)().api.src, 'server')));
    },
    task: (_ctx, task) => {
  ...
  const prismaTasks = [{
    ...
    task: (_ctx, task) => {
      ...
      const schemaContent = _fs.default.readFileSync(schemaPath, {
        encoding: 'utf-8',
        flag: 'r'
      });
