...
var _setupRealtime = require("./setupRealtime");
  ...
  const tasks = new _listr.Listr([{
    ...
    task: async (_ctx, task) => {
      const confirmation = await task.prompt({
        type: 'Confirm',
        message: 'Realtime is currently an experimental RedwoodJS feature. Continue?'
      });
      if (!confirmation) {
    ...
    task: () => {
      (0, _util.printTaskEpilogue)(_setupRealtime.command, _setupRealtime.description, _setupRealtime.EXPERIMENTAL_TOPIC_ID);
    }
  }], {
    rendererOptions: {
